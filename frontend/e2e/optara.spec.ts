...
test('mission control renders without browser errors',async({page})=>{
 const errors:string[]=[];page.on('pageerror',e=>errors.push(e.message));
 await page.goto('/');await expect(page.getByRole('heading',{name:'Mission Control',exact:true})).toBeVisible();
...
test('form drives backend SSE, graph state and evaluated result',async({page})=>{
 ...
 await expect(page.getByText('Why this recipe',{exact:true})).toBeVisible();
...
test('all product screens navigate and fit laptop widths',async({page})=>{
 await page.goto('/');
 for(const name of ['Runs','Shadow Lab','Calibration','Policies','Benchmarks','Mission Control']){
